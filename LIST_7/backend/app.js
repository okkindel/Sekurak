  ...
  try {
    jwt.verify(req.headers.authorization, 'zagor', function (err, decoded) {
      if (req.body.from == decoded.user) {
        connection.query('INSERT INTO przelewy (nadawca, odbiorca, konto, kwota, data, accepted) VALUES (?, ?, ?, ?, ?, ?)', [req.body.from, req.body.to, req.body.account, req.body.ammount, Date.now(), false], function (err, rows) {
          if (err) {
            res.status(500).json({
              status: 'Something went wrong.'
            });
